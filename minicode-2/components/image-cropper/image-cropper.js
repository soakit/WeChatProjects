Component({
  ...
  attached() {
    ...
    console.log('华丽的分割线-=-=-=-=-=-=-=-=-=-=-4')
    this._initCanvas().then(
      res => {
        console.log('华丽的分割线-=-=-=-=-=-=-=-=-=-=-', this.data.crx)
  ...
  methods: {
    ...
    _initCanvas() {
      return new Promise((resolve, reject) => {
        ...
        if (!this.data.ctx){
          let that = this
          console.log('华丽的分割线-=-=-=-=-=-=-=-=-=-=-6')
          wx.createSelectorQuery().in(this).select('.image-cropper-canvas').context(function (res) {
            console.log('华丽的分割线-=-=-=-=-=-=-=-=-=-=-7')
            that.data.ctx = res.context
            resolve(true)
          }).exec();
        }
      })
